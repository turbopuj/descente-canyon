...
$url_lieu='https://www.descente-canyon.com/canyoning/lieu/01025/Haute-Savoie.html';
...
$lieu = @file_get_contents($url_lieu, false, stream_context_create($arrContextOptions));
$extrait=extract_info($lieu,'var rows = ',', searchNom =');


preg_match_all('/canyon\/([0-9]+)\//U',$extrait,$results);
...
foreach ($results[1] as $key => $id) {
	...
	header("Content-Disposition: attachment; filename=canyon_hautesavoie.gpx");
	readfile($temp_file);
	unlink($temp_file);
